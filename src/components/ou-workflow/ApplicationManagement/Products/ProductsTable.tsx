...
export default function ProductsTable({
  ...
  const [selectedProduct, setSelectedProduct] = useState<any | null>(null); // null = adding, otherwise editing
  const queryClient = useQueryClient();

  const productData = application.products || [];
  const totalProducts = productData.length;
  const consumerProducts = productData.filter(
    (p) => p.consumerIndustrial === "C"
  ).length;
  const bulkShipped = productData.filter((p) => p.bulkShipped === "Y").length;
  const fromApplication = productData.filter(
    (p) => p.source === "Form Data"
  ).length;
  ...
  return (
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => {
            setSelectedProduct(null); // null means adding new product
                ...
                <tr key={index} className="border-b hover:bg-gray-50">
                  <td className="py-3 px-4">
                    <Badge
                      text={product.source}
                      color={
                        product.source === "Form Data" ? "orange" : "purple"
                      }
                    />
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <Badge
                      text={product.bulkShipped === "Y" ? "Yes" : "No"}
                      color={
                        product.bulkShipped === "Y" ? "green" : "gray"
                      }
                    />
                  </td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4 flex items-center space-x-2">
                    <button
                      ...
                      onClick={() => {
                        setSelectedProduct(product); // pass the product to edit
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <p className="text-sm text-blue-700">
          All products are submitted for OU Kosher certification. Product
          specifications and ingredient lists have been uploaded and verified.
          Manufacturing processes documented for each product category.
        </p>
