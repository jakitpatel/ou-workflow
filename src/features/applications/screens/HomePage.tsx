...
import { BarChart3, ClipboardList, PlusCircle, Trash2, ChevronRight } from 'lucide-react'
...
function DashboardCard({
  ...
  const content = (
    <>
      ...
      <h2
        className={`text-lg font-semibold ${iconColorClasses[variant]} mb-2 group-hover:underline`}
      >
        {title}
      </h2>
  ...
  return (
    <button type="button" onClick={onClick} className={commonClasses} aria-label={title}>
      {content}
    </button>
  )
}
...
export function HomePage() {
  ...
  const openDialog = useCallback(
    (mode: 'create' | 'create-intake' | 'delete' | 'delete-intake') => {
      if (canManage) setDialogMode(mode)
    },
    [canManage],
  )
  ...
  return (
    <PageShell className="bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      {/* Dialogs */}
      {canManage && dialogMode && (
        <DashboardAppDialog mode={dialogMode} isOpen={true} onClose={closeDialog} />
      )}
    </PageShell>
  )
}
